...
export async function GET(request: Request) {
  ...
  try {
    const [thisMonthRes, lastMonthRes] = await Promise.all([
      fetch(
        `${backendUrl}/analytics/period?account_id=a-001&period=thisMonth`,
        { next: { revalidate: 0 } },
      ),
      fetch(
        `${backendUrl}/analytics/period?account_id=a-001&period=lastMonth`,
        { next: { revalidate: 0 } },
      ),
    ]);
    ...
    const totalRevenue =
      Number(thisMonth.hero_sales ?? 0) + Number(lastMonth?.hero_sales ?? 0);
    ...
    const bestDay: string = (thisMonth.best_day ?? "Friday").replace(
      /\s*—.*$/,
      "",
    );
    const monthlyRevenue = Number(thisMonth.hero_sales ?? 0);

    const systemPrompt = `You are AlphaGrow, a financial eligibility engine for TNG eWallet Malaysia. You assess micro-merchant profiles and recommend the most suitable financing or growth program.
...
Score all 4 programs and recommend the best fit.`;
    ...
    if (!jsonMatch)
      throw new Error(`No JSON in response: ${rawText.slice(0, 200)}`);
    ...
    const isFood =
      category === "Food & Drinks" || category === "Products & Goods";
    const pkg: AiRecommendation["recommendedPackage"] = hasSSM
      ? isFood
        ? "B"
        : "track1"
      : "A";
    const tier: "1" | "2" =
      (pkg as string) === "track1" || (pkg as string) === "track2" ? "2" : "1";
    ...
    return Response.json({
      ...
      reasoning: [
        {
          label: "Scanned merchant transaction history",
          detail: "Transaction data retrieved from TNG account",
          icon: "bar",
        },
        {
          label: "Detected revenue growth pattern",
          detail: "Consistent income across 2 months of history",
          icon: "trend",
        },
        {
          label: "Verified business profile",
          detail: `SSM: ${hasSSM ? "confirmed" : "not registered"} · Category: ${category}`,
          icon: "shield",
        },
        {
          label: "Scored against all programs",
          detail: `${pkg === "B" ? "Package B" : pkg === "A" ? "Package A" : "Growth Track"} scored highest`,
          icon: "bar",
        },
        {
          label: "Recommendation finalised",
          detail: `Best match: ${pkg} based on revenue profile and SSM status`,
          icon: "trend",
        },
      ],
      metrics: {
