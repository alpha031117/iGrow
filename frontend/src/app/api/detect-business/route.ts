...
export async function GET() {
  try {
    ...
    if (!thisMonthRes.ok)
      throw new Error(`Analytics fetch failed: ${thisMonthRes.status}`);
    ...
    const totalSales =
      (thisMonth.hero_sales ?? 0) + (lastMonth?.hero_sales ?? 0);
    ...
    const systemPrompt = `You are AlphaGrow, an AI financial assistant embedded in TNG eWallet Malaysia. You analyse transaction patterns to detect whether a personal account is being used for micro-business activity.
...
Assess: does this look like micro-business activity on a personal account?`;
    ...
    return Response.json({
      summary:
        "We noticed frequent small incoming payments arriving in the evenings — this pattern closely matches how food business owners use TNG to collect payments.",
      confidence: "medium",
    });
  }
}
