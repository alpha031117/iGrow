...
const SUGGESTIONS = [
  ...
  },
  {
    ...
    prompt: "Help me create a realistic monthly budget. Where do I start?",
  ...
  },
  {
    ...
    prompt:
      "How do I calculate whether a loan is worth taking? Walk me through the math.",
    color:
  ...
  },
  {
    ...
    prompt: "What are the best strategies to grow my savings faster?",
    color:
...
type ChatAttachment =
  | { type: "image"; previewUrl: string; name: string }
  ...
  return msgs.map((m) => {
    if (m.role === "user") {
      ...
      if (m._imageData)
        parts.push({
          type: "image",
          data: m._imageData.base64,
          mimeType: m._imageData.mimeType,
        });
      return {
        role: "user",
        content:
...
async function processAttachment(attachment: FileAttachment): Promise<{
  ...
  if (file.type.startsWith("image/")) {
    ...
    const base64 = (attachment.previewUrl ?? "").replace(
      /^data:[^;]+;base64,/,
      "",
    );
    return {
  ...
  async function send(text?: string) {
    ...
    if (attachment) {
      try {
        ...
        if (result.textInjection)
          finalContent =
            result.textInjection + (rawText || "Please analyze this file.");
      } catch (err) {
    ...
    try {
      const res = await fetch("/api/chat", {
        ...
        body: JSON.stringify({
          messages: toPiMessages(nextForApi, modelId),
    ...
    <div className="flex flex-col h-screen bg-zinc-50 dark:bg-[#212121]">
      ...
      <header className="flex items-center justify-between border-b border-zinc-200 dark:border-zinc-800 px-6 py-3 bg-white dark:bg-[#2a2a2a] shrink-0">
        <div className="flex items-center gap-3">
          <div className="flex h-8 w-8 items-center justify-center rounded-xl bg-emerald-500 text-white text-sm font-bold shadow-sm">
            AG
          </div>
        ...
        {isEmpty ? (
          <div className="flex flex-col flex-1 items-center justify-center px-4 pb-4">
            <div className="flex flex-col items-center mb-10">
              <div className="flex h-16 w-16 items-center justify-center rounded-2xl bg-emerald-500 text-white text-2xl font-bold shadow-lg mb-5">
                iG
              </div>
              <h1 className="text-3xl font-semibold text-zinc-900 dark:text-zinc-50 text-center">
                How can I help you?
              </h1>
            ...
            <div className="grid grid-cols-2 gap-3 w-full max-w-2xl mb-8">
              {SUGGESTIONS.map((s) => (
                ...
                >
                  ...
                  <div>
                    <p className="font-medium text-sm text-zinc-900 dark:text-zinc-50 group-hover:text-emerald-600 dark:group-hover:text-emerald-400 transition-colors">
                      {s.title}
                    </p>
            ...
            <div className="flex-1 overflow-y-auto px-4 py-6">
              <div className="max-w-2xl mx-auto space-y-6">
                {messages.map((msg, i) => (
                  ...
                  >
                    {msg.role === "assistant" && (
                      <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-emerald-500 text-white text-xs font-bold mt-0.5 shadow-sm">
                        iG
                      </div>
                    )}
                    <div
                      className={`max-w-[80%] space-y-2 ${msg.role === "user" ? "items-end flex flex-col" : ""}`}
                    >
                      {/* Attachment preview */}
                      {msg.attachment &&
                        (msg.attachment.type === "image" ? (
                          <img
                            ...
                            className="max-h-48 rounded-2xl rounded-tr-sm object-cover shadow-sm"
                          />
                        ) : (
                            ...
                            <ReactMarkdown
                              components={{
                                ...
                                code: ({ children, className }) =>
                                  className ? (
                                    <code className="block my-3 rounded-lg bg-zinc-100 dark:bg-zinc-900 px-3 py-2.5 font-mono text-xs overflow-x-auto">
                                      {children}
                                    </code>
                                  ) : (
                                    <code className="rounded bg-zinc-100 dark:bg-zinc-900 px-1 py-0.5 font-mono text-xs">
                                      {children}
                                    </code>
                                  ),
                                pre: ({ children }) => <>{children}</>,
                                h1: ({ children }) => (
                                  <h1 className="mt-4 mb-2 text-base font-bold">
                                    {children}
                                  </h1>
                                ),
                                h2: ({ children }) => (
                                ...
                                blockquote: ({ children }) => (
                                  <blockquote className="my-3 border-l-2 border-zinc-300 dark:border-zinc-600 pl-3 italic text-zinc-500 dark:text-zinc-400">
                                    {children}
                                  </blockquote>
                                ),
                              }}
            ...
            <div className="px-4 pb-4 shrink-0">
              <div className="max-w-2xl mx-auto">
                ...
                <p className="text-center text-[11px] text-zinc-400 dark:text-zinc-600 mt-2">
                  AlphaGrow can make mistakes. Always verify financial
                  decisions.
                </p>
              </div>
            </div>
