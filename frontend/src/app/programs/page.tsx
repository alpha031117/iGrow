...
import {
  ArrowLeft, Banknote, Sparkles, CheckCircle2, ChevronRight,
  Star, ExternalLink, Info, Bot, TrendingUp, ShieldCheck,
      ...
      <div className="fixed inset-0 z-50 flex items-end justify-center"
        ...
        >
          ...
          <div className="flex flex-col items-center text-center gap-3 py-6">
            ...
            <h2 className="text-[#0D2B6E] text-[20px] font-bold">Expression of Interest Sent</h2>
            <p className="text-gray-500 text-[13px] leading-relaxed max-w-xs">
              A TNG advisor will review your profile and reach out within 2 business days.
            </p>
            <div className="w-full bg-[#EEF2FB] rounded-2xl p-4 text-left mt-2">
              <p className="text-[11px] font-bold uppercase text-[#1A5FD5] mb-2">What happens next</p>
              {["TNG pre-fills your application form", "Partner notified with your merchant data", "You'll receive an in-app notification"].map((s) => (
    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center"
      ...
      >
        ...
        <div className="flex items-center gap-3 mb-5">
          ...
          <div>
            <h2 className="text-[#0D2B6E] text-[16px] font-bold leading-snug">Apply — {program.name}</h2>
        ...
        <div className="bg-[#EEF2FB] rounded-2xl p-4 mb-4">
          <div className="flex items-center gap-1.5 mb-2">
            ...
            <span className="text-[11px] font-bold text-[#1A5FD5] uppercase tracking-wide">Pre-filled from your TNG account</span>
          </div>
          {[
            ...
            <div key={label} className="flex justify-between items-center py-1.5 border-b border-[#E5EBF8] last:border-0">
              <span className="text-[12px] text-gray-500">{label}</span>
              <span className="text-[12px] font-semibold text-[#0D2B6E]">{value}</span>
            </div>
          ))}
        </div>

        <p className="text-[12px] text-gray-400 leading-relaxed mb-5">
          By submitting, you authorise TNG to share your merchant data with this program partner to assess your eligibility. This is not a loan approval.
        </p>
...
export default function ProgramsPage() {
  ...
  useEffect(() => {
    ...
    if (window.location.hash === "#recommended") {
      setTimeout(() => {
        document.getElementById("recommended-program")?.scrollIntoView({ behavior: "smooth", block: "start" })
      }, 300)
    }
  }, [])
    ...
    <div className="min-h-screen flex flex-col" style={{ backgroundColor: "#EEF2FB", fontFamily: "Inter, sans-serif" }}>
      ...
      >
        <div className="max-w-lg mx-auto">
          <button onClick={() => router.back()} className="text-white/70 hover:text-white mb-5 flex items-center gap-1">
      ...
      <div className="w-full max-w-lg mx-auto px-4 py-5 flex flex-col gap-3">
            ...
            >
              ...
              >
                <div className="w-10 h-10 rounded-xl flex items-center justify-center shrink-0 mt-0.5"
                  style={{ background: isRecommended
                    ? "linear-gradient(135deg, #1A5FD5 0%, #0D2B6E 100%)"
                    : "#EEF2FB" }}>
                  {program.icon === "banknote"
                    ? <Banknote className={`w-5 h-5 ${isRecommended ? "text-white" : "text-[#1A5FD5]"}`} />
                    : <Sparkles className={`w-5 h-5 ${isRecommended ? "text-white" : "text-[#7C3AED]"}`} />}
                </div>

                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-0.5 flex-wrap">
                    <span className="text-[10px] font-bold uppercase tracking-wide"
                      style={{ color: program.track === "1" ? C.pri : "#7C3AED" }}>
                      {program.trackLabel}
                    </span>
                    {isRecommended && (
                  ...
                  <p className="text-[14px] font-bold text-[#0D2B6E] leading-snug">{program.name}</p>
                  <p className="text-[11px] text-gray-400 mt-0.5 leading-snug">{program.tagline}</p>
                </div>
              ...
              <AnimatePresence initial={false}>
                {isExpanded && (
                  ...
                  >
                    ...
                    {program.loanRange && (
                      <div 
                        className="grid grid-cols-2 gap-2 mt-3 mb-3"
                      >
                        {[
                          { label: "Loan range", value: program.loanRange },
                          { label: "Revenue deduction", value: program.deduction! },
                          { label: "Repayment", value: program.repayment! },
                          { label: "Review trigger", value: program.reviewTrigger! },
                        ].map(({ label, value }) => (
                          <div key={label} className="bg-[#EEF2FB] rounded-xl p-2.5">
                            <p className="text-[10px] text-gray-400 mb-0.5">{label}</p>
                            <p className="text-[12px] font-semibold text-[#0D2B6E] leading-snug">{value}</p>
                          </div>
                        ))}
                      </div>
                    )}
                    ...
                    {/* Highlights */}
                    <div>
                      <p className="text-[10px] font-bold uppercase tracking-wide text-gray-400 mb-2 mt-1">What you get</p>
                      <div className="flex flex-col gap-1.5 mb-3">
                        {program.highlights.map(h => (
                          <div key={h} className="flex items-start gap-2">
                            <CheckCircle2 className="w-3.5 h-3.5 text-[#1A5FD5] shrink-0 mt-0.5" />
                            <span className="text-[12px] text-[#374151] leading-snug">{h}</span>
                          </div>
                        ))}
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      <div className="flex flex-col gap-1.5 mb-4">
                        {program.eligibility.map(e => (
                          <div key={e} className="flex items-start gap-2">
                            ...
                            <span className="text-[12px] text-gray-500 leading-snug">{e}</span>
                          </div>
                        ))}
                      </div>
                      ...
                      >
                        {isRecommended ? "Apply — Recommended for You" : "Apply for This Program"}
        ...
        <p className="text-center text-[11px] text-gray-400 pb-6 pt-2 px-4 leading-relaxed">
          All programs are subject to individual eligibility review. This is not a guarantee of approval.
        </p>
      </div>
