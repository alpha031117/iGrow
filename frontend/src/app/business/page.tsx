...
// Deterministic mock QR payload — a TNG-style deep-link string
...
// Minimal SVG QR grid — renders a fixed 21×21 pattern that looks like a real QR
// (not scannable — purely visual mock)
function MockQrSvg({ size = 200 }: { size?: number }) {
  // Fixed bit pattern for a visually authentic QR
  const cells = [
        ...
        <div className="flex gap-3 px-6 pb-7">
          <button
            ...
            onClick={() => {
              // Mock share
              if (navigator.share) {
                navigator.share({ title: `${businessName} — TNG Merchant QR`, text: mockQrPayload(businessName) })
              }
            }}
          >
...
export default function BusinessPage() {
  ...
  const checklist = [
    { task: "Confirm business name", done: true },
    { task: "Set up TNG Business Account", done: true },
    { task: "Generate Merchant QR code", done: qrGenerated },
    { task: "Complete first 10 QR payments", done: false },
    { task: "Prepare SSM registration", done: false },
  ]
              ...
              <div className="relative w-16 h-16">
                <svg className="w-full h-full -rotate-90" viewBox="0 0 64 64">
                  ...
                  <circle
                    ...
                    strokeDashoffset={`${2 * Math.PI * 26 * (1 - 0.35)}`}
                    strokeLinecap="round"
                  />
                </svg>
                <div className="absolute inset-0 flex items-center justify-center">
                  <span className="text-white font-bold text-[13px]">35%</span>
                </div>
              </div>
      ...
      <div className="w-full max-w-lg mx-auto px-4 -mt-4 flex flex-col gap-4 pb-8">
        ...
        <div className="bg-white rounded-3xl p-4 shadow-md border border-[#E5EBF8]">
          ...
          <div className="w-full h-1.5 bg-[#EEF2FB] rounded-full mb-4">
            <div
              ...
              style={{ width: `${(doneCount / checklist.length) * 100}%`, backgroundColor: "#1A5FD5" }}
            />
          </div>
            ...
            {checklist.map((item, i) => (
              <div key={i} className="flex items-center gap-3">
                ...
                <span
                  ...
                  style={{ color: item.done ? "#0D2B6E" : "#6B7280", fontWeight: item.done ? 600 : 400 }}
                >
                  {item.task}
                </span>
              </div>
        ...
        <div className="bg-white rounded-3xl p-4 shadow-md border border-[#E5EBF8]">
          ...
          <div className="w-full h-2.5 bg-[#EEF2FB] rounded-full mb-1 overflow-hidden">
            <div className="h-full rounded-full" style={{ width: "35%", background: "linear-gradient(90deg, #1A5FD5, #6EE7B7)" }} />
          </div>
          <div className="flex justify-between text-[11px] text-gray-400 mb-4">
            <span>0</span>
            <span className="font-semibold text-[#1A5FD5]">35 / 100</span>
            <span>100</span>
          </div>
