...
function getIconForCategory(categoryId: string, credit: boolean) {
  ...
  if (categoryId === 'c-002' || categoryId === 'c-008') return { Icon: Car, iconBg: '#0D2B6E' }
...
function groupByDate(txns: { id: string; title: string; amount: number; direction: 'credit' | 'debit'; transactedAt: string; category: { id: string; name: string } }[]): TxGroup[] {
  ...
  for (const tx of txns) {
    ...
    if (d.getTime() === today.getTime()) {
      ...
      label = d.toLocaleDateString('en-MY', { day: 'numeric', month: 'long', year: 'numeric' })
    }
...
export default function TransactionsPage() {
  ...
  const [total, setTotal] = useState(fallbackGroups.reduce((n, g) => n + g.txns.length, 0))
            ...
            {groups.map((group, gi) => (
              <div key={group.label} className={gi > 0 ? 'mt-2' : ''}>
                <div className="flex items-center justify-between px-1 py-1">
                  <span className="text-[#1A5FD5] text-[13px] font-bold uppercase tracking-wide">{group.label}</span>
                  <span className="text-[#6B7280] text-[12px]">{group.txns.length} transactions</span>
                </div>
                {group.txns.map((tx) => (
