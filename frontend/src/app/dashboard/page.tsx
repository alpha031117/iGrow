    ...
    >
      ...
      >
        ...
        <button onClick={onChat}
          ...
          <ExternalLink className="w-4 h-4" /> Ask AlphaGo how to register SSM
        </button>
      ...
      <div className="w-full max-w-lg bg-white rounded-t-3xl px-5 pt-5 pb-8 shadow-2xl overflow-y-auto max-h-[92vh]"
        ...
        <div className="flex items-center gap-2 mb-4">
          ...
          <div>
            ...
            <p className="text-[11px] text-gray-400">AlphaGo Analytics Engine · merchant profile analysis</p>
          </div>
        </div>
    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center" style={{ backgroundColor: "rgba(0,0,0,0.6)" }}>
      <div className="w-full max-w-lg bg-white rounded-t-3xl px-5 pt-6 pb-8 shadow-2xl">
        ...
        <div className="flex items-center gap-3 mb-5">
          ...
          <div>
            <p className="text-[#0D2B6E] text-[15px] font-bold leading-snug">AlphaGo Analytics Engine</p>
...
function ReportSheet({ open, onClose, activeData }: { open: boolean; onClose: () => void; activeData: PeriodData }) {
  ...
  async function genPDF(type: "weekly" | "monthly" | "annual") {
    ...
    doc.text("AlphaGo Insight", M + 6, y + 7)
    doc.setFont("helvetica", "normal"); doc.setFontSize(8); doc.setTextColor(100, 116, 139)
    const insightText = d.tip.body.replace(/<[^>]+>/g, "")
    doc.text(doc.splitTextToSize(insightText, CW - 12), M + 6, y + 12)

    doc.setFontSize(7); doc.setTextColor(148, 163, 184)
    doc.text("Generated by AlphaGo · Powered by TNG", W / 2, 285, { align: "center" })
    doc.save(`AlphaGo-${tl.replace(" ", "-")}-${new Date().toISOString().slice(0, 10)}.pdf`)
...
export default function DashboardPage() {
  ...
  return (
    <div className="min-h-screen pb-8" style={{ backgroundColor: C.g50, fontFamily: "Inter, sans-serif", maxWidth: 430, margin: "0 auto" }}>
      ...
      {launchpadAccepted && recommendedTier !== "" && (() => {
        ...
        return (
          <Card id="launchpad-card">
            ...
            <p className="text-[11px] text-gray-400 mb-2">AlphaGo Analytics Engine · merchant profile analysis</p>
            <div className="flex items-center gap-2 mb-4">
              <div className="flex-1 h-1.5 rounded-full bg-[#E5EBF8] overflow-hidden">
      ...
      <p className="text-center text-[11px] font-medium pb-4" style={{ color: C.g400 }}>AlphaGo · Powered by TNG</p>
